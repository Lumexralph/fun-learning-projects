...
function CartDAO(database) {
    ...
    this.getCart = function(userId, callback) {
        ...
        *
        */
        collection.find( { userId: userId } ).toArray((error, userCart) => {
    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         *
         */
        collection.find( { userId: userId } ).project( {items: 1, _id: 0} ).toArray((error, userCart) => {
            assert.equal(error, null);
            //console.log(userCart[0].items.find((el, index, array) => el._id === itemId ));
            //check if item is in cart
           let itemInCart = userCart[0].items.find((el, index, array) => el._id === itemId );
            callback( itemInCart ? itemInCart : null );   
            console.log('To know item in cart : ' + itemInCart );
