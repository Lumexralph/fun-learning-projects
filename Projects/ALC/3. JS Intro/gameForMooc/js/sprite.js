...
function Sprite() {
    ...
    this.extractSprites = function (spritesheet, nbPostures, postureToExtract, nbFramesPerPosture,
        ...
        var postureWidth = nbFramesPerPosture * spriteWidth;

        if (postureWidth < spritesheet.width) {
            nbSpritesPerRow = nbFramesPerPosture;
        } else {
            nbSpritesPerRow = Math.floor(spritesheet.width / spriteWidth);
        }

        // Extract each sprite
        var startIndex = postureToExtract * nbFramesPerPosture;
